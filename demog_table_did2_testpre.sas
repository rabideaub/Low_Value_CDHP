...
data optum_demog (keep=Patid Fst_dt gdr_cd age age_cat product cdhp lv D_EDUCATION_LEVEL_CODE D_RACE_CODE 
	...
	set in.low_value_beneficiary (where=((year=2011 & 17<=age<=62) | (year=2012 & 18<=age<=63))); 

	if 17<=age<=34 then age_cat='18-34';
...
data elig2011 elig2012 elig2013;
	set elig_flags;
	output elig2011;
	output elig2012;
	output elig2013;
...
data elig;
	...
	if (year=2011 & 17<=age<=62) | (year=2012 & 18<=age<=63);
	if treatment~=.;
run;
...
title "Check count per year";
proc freq data=elig;
	tables treatment*year / missing;
run;
title;

proc print data=optum_demog (obs=12);
	var patid year;
run;

proc print data=elig (obs=12);
	var patid year product:;
run;
...
data optum_demog;
	...
	if upcase(product)='IND' then product='PPO'; /*Combine PPO and Indemnity*/
	if upcase(product)='OTH' then product='POS'; /*Combine POS and Other*/
	if upcase(product_2011)='IND' then product_2011='PPO'; /*Combine PPO and Indemnity*/
	if upcase(product_2011)='OTH' then product_2011='POS'; /*Combine POS and Other*/
	if upcase(product_2012)='IND' then product_2012='PPO'; /*Combine PPO and Indemnity*/
	if upcase(product_2012)='OTH' then product_2012='POS'; /*Combine POS and Other*/
	if upcase(product_2013)='IND' then product_2013='PPO'; /*Combine PPO and Indemnity*/
	if upcase(product_2013)='OTH' then product_2013='POS'; /*Combine POS and Other*/
	if product_2011 in ('PPO','POS') & product_2012 in('PPO','POS') & product_2013 in('PPO','POS'); /*Only keep bene's that are in PPOs or POS for all years*/
...
data in.beneyear_cdhp_did_saf_match_pre;
	set optum_demog_match;
run;

proc freq data=in.beneyear_cdhp_did_saf_match_pre;
	tables gdr_cd age_cat product D_RACE_CODE D_HOUSEHOLD_INCOME_RANGE_CODE division charlson_count / missing;
run;

proc export data=in.beneyear_cdhp_did_saf_match_pre
			outfile = "/schaeffer-a/sch-projects/dua-data-projects/OPTUM/rabideau/Data/CDHP/beneyear_cdhp_did_saf_match_pre.dta" replace;
run;
