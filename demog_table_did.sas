...
proc print data=optum_demog (obs=10);
	var CVPRIO01-CVPRIO18 CVINDX01-CVINDX18 charlson_count;
run;
...
title "Charlson by Treatment Before Matching";
proc freq data=optum_demog;
...
proc freq data=optum_demog;
	tables gdr_cd age_cat product D_RACE_CODE D_HOUSEHOLD_INCOME_RANGE_CODE division charlson_count treatment year treatment*year / missing;
run;
...
proc freq data=optum_demog2012;
	tables treatment cdhp_cd charlson_count*treatment;
run;

proc freq data=optum_demog2013;
	tables treatment cdhp_cd charlson_count*treatment;
run;
...
proc freq data=optum_demog;
	tables year treatment*cov_only charlson_count*treatment product*treatment / missing;
run;
...
proc export data=in.beneyear_cdhp_did_saf_match
			outfile = "/schaeffer-a/sch-projects/dua-data-projects/OPTUM/rabideau/Data/CDHP/beneyear_cdhp_did_saf_match_q2q3.dta" replace;
run;
...
ods tagsets.excelxp file="&out./cdhp_did_demog_noro_match.xml" style=sansPrinter;
