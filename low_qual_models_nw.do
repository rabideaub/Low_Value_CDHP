...
foreach depvar in $varlist {;
	use "/schaeffer-a/sch-projects/dua-data-projects/OPTUM/rabideau/Data/CDHP/beneyear_cdhp_did_saf_match_nw.dta",clear;
	describe, simple;
	...
	*sample 100000,count;
	...
	summarize cst_`depvar',detail;
		...
		/*Output to Excel*/
		esttab tpm_all_prop_adj tpm_all_lv_adj tpm_all_tot_adj tpm_l_sens_adj tpm_m_sens_adj  using "/schaeffer-a/sch-projects/dua-data-projects/OPTUM/rabideau/Output/cdhp_adj_`depvar'_nw.xls", 
		...
		esttab tpm_all_prop tpm_all_lv tpm_all_tot tpm_l_sens tpm_m_sens  using "/schaeffer-a/sch-projects/dua-data-projects/OPTUM/rabideau/Output/cdhp_`depvar'_nw.xls", 
			cells((margins_b(star fmt(4)) p(fmt(%4.3f)) ci_l(fmt(a3)) ci_u(fmt(a3))) margins_se(par(`"("' `")"') fmt(4))) /*These are parentheses that excel can see. otherwise you can just specify par*/
			legend label varlabels(_cons Constant) mtitles("LV `depvar' Cost Ratio" "LV `depvar' Costs" "All `depvar' Costs")
			extracols(5)
			stats(N, fmt(0 3) labels(`"Observations"')) replace;
